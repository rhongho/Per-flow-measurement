...
hashtable_L2* ht_create_L2(uint32_t size) {
	...
	if ((table = (hashtable_L2*)malloc(sizeof(hashtable_L2))) == NULL)
	{
		return NULL;
	}
	table->size = size;
...
hashtable_L3* ht_create_L3( uint32_t size) {
	...
	/* Allocate the table itself. */
	if ((table = (hashtable_L3*)malloc(sizeof(hashtable_L3))) == NULL)
	{
		return NULL;
	}
...
void ht_insert_L3(hashtable_L3 *hashtable, uint32_t hash_value, uint32_t is, uint32_t id, double est) {
	...
	for (;;)
	{
		...
		if (me == NULL)
		{
			...
			++(hashtable->usage);
			//if(hashtable->usage%100000==0)
			//	printf("%f\n",(double)hashtable->usage/hashtable->size);
...
hashtable_L4 *ht_create_L4( uint32_t size) {
	...
	if ((table = (hashtable_L4*)malloc(sizeof(hashtable_L4))) == NULL)
	{
		return NULL;
	}
